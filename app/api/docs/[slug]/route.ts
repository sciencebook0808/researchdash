...
export async function PATCH(req: Request, { params }: { params: { slug: string } }) {
  ...
  const current = await prisma.documentationPage.findUnique({ where: { slug: params.slug } })
  if (current && body.content && body.content !== current.content) {
    const versionCount = await prisma.docVersion.count({ where: { pageId: current.id } })
...
export async function DELETE(req: Request, { params }: { params: { slug: string } }) {
  await prisma.documentationPage.delete({ where: { slug: params.slug } })
